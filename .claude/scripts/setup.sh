...
CAVEMAN_SL="${caveman_statusline:-}" python3 - "$statusline_script" <<'PY'
import os, sys, stat, pathlib

sl_path = pathlib.Path(sys.argv[1])
caveman_path = os.environ.get("CAVEMAN_SL", "")

# Build caveman section — call the plugin script if it exists, otherwise skip
if caveman_path:
    caveman_block = f'CAVEMAN=$(bash "{caveman_path}" <<< "")'
else:
    caveman_block = 'CAVEMAN=""'

sl_path.write_text(f'''#!/usr/bin/env bash
# Claude Code statusline: caveman badge + git branch + via tool + Claude context
# Auto-generated by setup.sh — edits will be overwritten on next run.
...
''')
sl_path.chmod(sl_path.stat().st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
print(f"✓ Wrote {sl_path}")
PY

# Point global settings.json at the wrapper script
